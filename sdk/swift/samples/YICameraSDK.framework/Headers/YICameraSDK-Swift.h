...
/**
  ...
  operating an camera, App need invoke \code
  ActionCamera.connect(connectionString:)
  \endcode to connect to the camera.
  After connect success (\code
  ActionCameraListener.onConnected()
  \endcode will be invoked), app can invoke other functions.
  Whenever app wants to disconnect from the camera, app can invoke \code
  ActionCamera.disconnect()
  \endcode.
  Any \code
  success
  \endcode or \code
  fail
  \endcode callback in commands are optional parameter. If you don’t care command
  execution result, you can pass \code
  nil
  \endcode.
*/
...
/**
  ...
  Invoke this function to connect to a YI action camera. Param \code
  connectionString
  \endcode represents
  ...
  \code
  ActionCameraListener.onConnected()
  \endcode callback will be invoked. If fail,
  \code
  ActionCameraListener.onClosed(error:)
  \endcode callback will be invoked and parameter \code
  error
  \endcode
  will be set.
*/
...
*/
- (CIImage * _Nullable)buildLiveVideoQRCodeWithSsid:(NSString * _Nonnull)ssid password:(NSString * _Nonnull)password resolution:(enum LiveVideoResolution)resolution bitrate:(enum LiveVideoBitrate)bitrate duration:(NSInteger)duration rtmpUrl:(NSString * _Nonnull)rtmpUrl error:(NSError * _Nullable * _Nullable)error;
...
/**
  ...
  commands will be cancelled (the \code
  fail
  \endcode callback of the command will be invoked and the
  code of \code
  error
  \endcode parameter will be set to \code
  ErrorCode.Cancelled
  \endcode). At the end,
  \code
  ActionCameraListener.onClosed(error:)
  \endcode callback will be invoked and \code
  error
  \endcode parameter
  will be set to \code
  nil
...
*/
- (ActionCamera * _Nonnull)getPhotoResolutionWithSuccess:(void (^ _Nullable)(enum PhotoResolution))success fail:(void (^ _Nullable)(YICameraSDKError * _Nonnull))fail;
@end
...
*/
- (ActionCamera * _Nonnull)getStatusWithSuccess:(void (^ _Nullable)(enum CameraStatus))success fail:(void (^ _Nullable)(YICameraSDKError * _Nonnull))fail;
@end
...
*/
- (ActionCamera * _Nonnull)getVideoResolutionWithSuccess:(void (^ _Nullable)(enum VideoResolution))success fail:(void (^ _Nullable)(YICameraSDKError * _Nonnull))fail;
@end
...
/**
  ...
  <em>Note: This method must be thread safe and \code
  task
  \endcode must be executed out of current call stack.</em>
